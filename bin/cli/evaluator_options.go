...
type EvaluatorOptions struct {
	...
	// and load resolved dependencies from a Project.json.json file inside this directory.
	//
	// NOTE:
	// Setting this option is not equivalent to setting the `--project-dir` flag from the CLI.
	// When the `--project-dir` flag is set, the CLI will evaluate the Project.pkl file,
	// and then applies any evaluator settings and dependencies set in the Project.pkl file
	// for the main evaluation.
	//
	// In contrast, this option only determines how Pkl considers whether files are part of a
	// project.
	// It is meant to be set by lower level logic in Go that first evaluates the Project.pkl,
	...
	//
	// When importing dependencies, a Project.json.json file must exist within ProjectDir
...
func (e *EvaluatorOptions) project() *msgapi.ProjectOrDependency {
	...
	return &msgapi.ProjectOrDependency{
		ProjectFileUri: fmt.Sprintf("file://%s/Project.pkl", e.ProjectDir),
...
var WithProjectDependencies = func(project *Project) func(opts *EvaluatorOptions) {
	return func(opts *EvaluatorOptions) {
		opts.ProjectDir = strings.TrimPrefix(strings.TrimSuffix(project.ProjectFileUri, "/Project.pkl"), "file://")
